...
If Request.QueryString("DelID")<>"" Then
	...
	rs.open "Select id,Num From p8_Class Where id= " & id ,Conn,1,3
		...
		'检查是否有图片，如果有则无法删除
		Set rsfin = Server.Createobject("Adodb.RecordSet")
		rsfin.open "Select id From p8_Pic Where BigClass = " & Clng(id) & " Or SmallClass = " & Clng(id) & "",Conn,1,1
		If Not rsfin.Eof Then
			Response.Write "<script>alert(""请先删除分类下的图片"");history.back()</script>"
			Response.End()
		End If
		rsfin.Close
		Set rsfin = Nothing	
	...
	rs.Close
	Set rs = Nothing
	Response.Redirect "Class_Pic_List.asp?Tip=删除成功！"
	Response.End()
End If
'/删除============================================================================================
%>
  ...
  <tr>
    <td bgcolor="#F8FBFE"><table border="0" cellspacing="10" cellpadding="0">
      <tr>
        ...
        <td width="80" height="30" align="center" class="Tab1_over">图片分类</td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="5">
  <tr bgcolor="#F8FBFE">
    <td height="25" bgcolor="#F8FBFE">&nbsp;<span class="f14 cBlack">图片列表</span>&nbsp;&nbsp;&nbsp;<a href="Class_Pic_Add.asp">增加子类</a></td>
  </tr>
...
<%
	...
	Set rs = Server.CreateObject("ADODB.RecordSet")
	rs.open "Select id,ClassName,ParentID,ClassLevel From p8_Class Where ClassType='图片' And ClassLevel=1 Order By id Desc",Conn,1,1
...
%>
		<tr style="<%=ico%>" onMouseOver="this.style.backgroundColor='#e7ffa6'" onMouseOut="this.style.backgroundColor='#F8FBFE'">
		  <td height="25" style="padding-left:60px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
			  <td width="200" class="cBlack"><a href="Class_Pic_Edit.asp?id=<%=rs("id")%>" class="cBlack"><%=rs("ClassName")%></a></td>
			  <td><a href="Class_Pic_Add.asp?ParentID=<%=rs("id")%>&ClassLevel=<%=rs("ClassLevel")%>">增加子类</a>&nbsp;| <a href="Class_Pic_Edit.asp?id=<%=rs("id")%>">修改</a>&nbsp;|&nbsp;<a href="javascript:if(confirm('删除后不可恢复，是否继续？'))window.location.href='?DelID=<%=rs("id")%>';">删除</a></td>
			</tr>
		  </table></td>
		</tr>
		<%
			...
			rsnxt.open "Select id,ClassName,ParentID,ClassLevel From p8_Class Where ClassType='图片' And ClassLevel=2 And ParentID="& rs("id") &" Order By id Desc",Conn,1,1
		...
		%>
				<tr style="<%=str%>" onMouseOver="this.style.backgroundColor='#e7ffa6'" onMouseOut="this.style.backgroundColor='#F8FBFE'">
				  <td height="25" style="padding-left:120px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
					  <td width="206" class="cBlack"><a href="Class_Pic_Edit.asp?id=<%=rsnxt("id")%>" class="cBlack"><%=rsnxt("ClassName")%></a></td>
					  <td><a href="Class_Pic_Edit.asp?id=<%=rsnxt("id")%>">修改</a>&nbsp;|&nbsp;<a href="javascript:if(confirm('删除后不可恢复，是否继续？'))window.location.href='?DelID=<%=rsnxt("id")%>';">删除</a></td>
					</tr>
				  </table></td>
				</tr>
